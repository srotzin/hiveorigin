...
export const PROV_SERVICE_DID = 'did:hive:hiveorigin';
...
export async function loadOrCreateKey() {
  ...
  if (envSeed) {
    try { const d = b64uDecode(envSeed); if (d.length === 32) seed = d; } catch (_) {}
  }
  if (!seed && existsSync(SEED_FILE)) {
    try { const r = readFileSync(SEED_FILE); if (r.length === 32) seed = r; } catch (_) {}
  }
  if (!seed) {
    seed = randomBytes(32);
    try { writeFileSync(SEED_FILE, seed, { mode: 0o600 }); } catch (_) {}
  }
...
export async function getPubkeyInfo() {
  ...
  return {
    issuer:      PROV_SERVICE_DID,
    algorithm:   'Ed25519',
    pubkey_b64u: b64uEncode(pubKey),
    pubkey_hex:  Buffer.from(pubKey).toString('hex'),
    doctrine:    'smash.prov — every door 200s, every byte signed',
  };
}
...
export async function signResponse(method, path, bodyBytes, ts) {
  ...
  return {
    'X-Hive-Prov-Iss':     PROV_SERVICE_DID,
    'X-Hive-Prov-Ts':      String(ts),
    'X-Hive-Prov-Sig':     b64uEncode(sigBytes),
    'X-Hive-Prov-Pubkey':  PROV_PUBKEY_PATH,
    'X-Hive-Prov-Payload': payload,
  };
}

const STAMP_EXACT = new Set([
  '/', '/health', '/llms.txt', '/robots.txt', '/sitemap.xml',
  '/favicon.ico', '/openapi.json', '/docs', '/redoc',
  '/.well-known/agent.json', '/v1/prov/pubkey',
]);
...
export function smashProvMiddleware(req, res, next) {
  ...
  res.send = async function provSend(body) {
    ...
    if (stamp) {
      try {
        let bodyBuf = Buffer.isBuffer(body) ? body
        ...
        res.setHeader('Access-Control-Expose-Headers',
          'X-Hive-Prov-Iss, X-Hive-Prov-Ts, X-Hive-Prov-Sig, X-Hive-Prov-Pubkey, X-Hive-Prov-Payload');
